...
interface PursuitContextType {
  ...
  answerQuestions: (answers: { question: string; answer: string }[]) => void;
}
...
export function PursuitProvider({ children }: { children: ReactNode }) {
  ...
  const nextStep = () => {
    if (state.currentStep < 4) {
      ...
      router.push(`/step/${nextStepNumber}`);
    }
  };
  ...
  const prevStep = () => {
    if (state.currentStep > 1) {
      ...
      dispatch({ type: "SET_CURRENT_STEP", payload: prevStepNumber });
      router.push(`/step/${prevStepNumber}`);
    }
  };
    ...
    try {
      ...
      dispatch({
        ...
        payload: {
          ...
          questions:
            questions.length > 0
              ? questions.map((q) => ({ question: q, reason: "" }))
              : undefined,
        },
      });
    } catch (error) {
