...
const ConceptSummaryStep: React.FC = () => {
  const { state, getCurrentVersion, setCurrentVersion, approveCurrentVersion, editCurrentVersion, answerQuestions, scrollToNextStep } = usePursuit();
  ...
  if (state.isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-full py-20">
        <div className="animate-pulse text-2xl text-primer-purple-light font-semibold mb-8">Thinking...</div>
        <svg className="animate-spin h-16 w-16 text-primer-purple" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        <p className="mt-8 text-gray-300">We're crafting your pursuit concept...</p>
      </div>
    );
  }
  ...
  if (!currentVersion) {
    return (
      <div className="flex flex-col items-center justify-center h-full py-20">
        <div className="text-2xl text-primer-purple-light font-semibold mb-4">No concept summary available</div>
        <p className="text-gray-300">Please go back and submit your idea first</p>
      </div>
    );
  }
  ...
  const meetsAllCriteria = () => {
    return currentVersion.scores.every(score => score.score === score.maxScore);
  };
    ...
    return (
      <div key={score.name} className="mb-4">
        <div className="flex justify-between mb-1">
          <span className="text-sm font-medium text-gray-300">{score.name}</span>
  ...
  return (
    <div className="flex flex-col space-y-8">
      <div className="text-center">
        <h1 className="text-3xl font-bold text-primer-purple-light mb-6">Concept Summary</h1>
        {versions.length > 1 && (
          <div className="mb-8">
        ...
        {isEditing ? (
          <div className="space-y-4">
            <h2 className="text-2xl font-semibold">
              {editMode === 'direct' ? 'Edit Summary Directly' : 'Give Instructions for Changes'}
            </h2>
            ...
            <div className="flex justify-end space-x-4">
              ...
              <button
                ...
                disabled={editMode === 'direct' ? !editedSummary.trim() : !editInstructions.trim()}
              >
                Save Changes
              </button>
            </div>
          ...
          <>
            ...
            {currentVersion.questions && currentVersion.questions.length > 0 ? (
              <div className="bg-primer-gray-dark p-6 rounded-lg">
                <h2 className="text-2xl font-semibold mb-4">Refinement Questions</h2>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-end">
                    <motion.button
                      ...
                      disabled={!answers.length || !answers.every(a => a.trim())}
                    >
                      Submit Answers
                    </motion.button>
                  </div>
